...
"""
reward_tracker.py
Tracks trade outcomes, calculates reward scores, and updates pattern stats in a local SQLite DB.
"""
import sqlite3
import os
import pandas as pd
...
# DB lives in the same directory as this script (src/)
...
def calculate_reward(pnl_pct, confidence, hold_minutes):
    """Calculates a reward score based on P&L, confidence, and hold time."""
    is_win = pnl_pct > 0
...
def log_trade(trade_data):
    """Logs a closed trade and updates rolling pattern statistics."""
    init_db()
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if pat:
            ...
            if row:
                ...
                avg_rr = avg_p / 8.0
                rr = 0.3 * reward + 0.7 * rr  # EMA for recent reward
                cur.execute("""
...
def get_recent_performance(hours=24):
    """Fetches aggregated performance metrics for the UI."""
    init_db()
