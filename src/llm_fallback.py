...
def query_ollama(prompt: str, model: str = "llama3.2:1b", timeout: int = 30) -> Optional[dict]:
    try:
        res = requests.post(OLLAMA_URL, json={"model": model, "prompt": prompt, "stream": False, "format": "json"}, timeout=timeout)
        res.raise_for_status()
        data = _parse_json(res.json().get("response", ""))
        if data and "action" in data:
            return {"source": "ollama", "decision": data.get("action", "WAIT"), "confidence": float(data.get("confidence", 0.5)), "reasoning": data.get("reasoning", "")}
    except Exception as e: print(f"[Ollama Error] {e}")
    return None

def query_cloud(prompt: str) -> Optional[dict]:
    if not CLOUD_API_URL or not CLOUD_API_KEY: return None
    try:
        headers = {"Authorization": f"Bearer {CLOUD_API_KEY}", "Content-Type": "application/json"}
        res = requests.post(CLOUD_API_URL, json={"model": "gpt-4o-mini", "messages": [{"role":"system","content":"Forex assistant. JSON only: {action,confidence,reasoning}"},{"role":"user","content":prompt}], "temperature": 0.1, "response_format": {"type": "json_object"}}, headers=headers, timeout=20)
        res.raise_for_status()
        data = _parse_json(res.json()["choices"][0]["message"]["content"])
        if data and "action" in data:
            return {"source": "cloud", "decision": data.get("action", "WAIT"), "confidence": float(data.get("confidence", 0.5)), "reasoning": data.get("reasoning", "")}
...
def get_decision_fallback(token_window: str, price: float, local_matches: list, config: dict) -> dict:
    pat_sum = "\n".join([f"- {m['pattern']}: WR={m['win_rate']:.2f}, RR={m['avg_rr']:.2f}" for m in local_matches[:3]]) if local_matches else "No patterns."
    prompt = f"EUR/USD 1-min trade.\nPrice: {price:.5f}\nSeq: {token_window}\nHistory:\n{pat_sum}\n\nJSON ONLY: {{\"action\": \"BUY/SELL/WAIT\", \"confidence\": 0.0-1.0, \"reasoning\": \"...\"}}"
    res = query_ollama(prompt, config.get("ollama_model", "llama3.2:1b"))
    if res and res["confidence"] >= 0.6: return res
    res = query_cloud(prompt)
    return res or {"source": "fallback", "decision": "WAIT", "confidence": 0.3, "reasoning": "Models uncertain"}#!/usr/bin/env python3
"""
llm_fallback.py
